...
const LawnReplEligibilityDialog = ({
  ...
  const prevLastTouchedIndex = useRef()
  ...
  const eligibleFieldsHaveError = useMemo(
    () =>
      [
        ...
        .filter((error) => !/required/i.test(error))
        .some(Boolean),
    [errors]
  )
  ...
  const stepHasError = useCallback(
    (fieldName: string) => {
      ...
      return Boolean(error) && !/required/i.test(error)
    },
    [errors]
  )
...
export default connect(
  withStyles(styles, {withTheme: true})(LawnReplEligibilityDialog)
)
...
const questionStyles: StyleRulesCallback = (theme) => ({
  qualifyMsg: {
    marginTop: theme.spacing.unit * 3
  }
})

const QuestionOne = withStyles(questionStyles)(({classes}) => {
  return (
...
const QuestionTwo = withStyles(questionStyles)(({classes}) => {
  return (
...
const QuestionThree = withStyles(questionStyles)(({classes}) => {
  return (
...
const QuestionFour = withStyles(questionStyles)(({classes}) => {
  return (
