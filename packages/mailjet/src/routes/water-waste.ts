...
import {MailJetSendRequest, MailJetAttachment} from '../lib/types'
...
const attach = (reqAttachments: string[]): Promise<MailJetAttachment[]> => {
  const attachments: MailJetAttachment[] = []
  return new Promise((resolve) => {
    ...
    reqAttachments.forEach(async (filename: string) => {
      ...
      try {
        ...
        const mimeTypeViaImage = image.getMIME()
        image
          .resize(800, Jimp.AUTO)
          .getBuffer(mimeType || mimeTypeViaImage, (err, buffer) => {
            bufferToBase64(err, buffer, filename, mimeType || mimeTypeViaImage)
          })
      } catch (error) {
        console.log(error)
        throw error
...
const waterWasteHandler = async (req: IncomingMessage) => {
  ...
  try {
    const sendAttachments: MailJetAttachment[] = await attach(attachments)
    // console.log("Promise resolved: ", val)
    // Just add the Inline_attachments property if we have attachments (seems optional, but more explicit).
    if (Object.keys(sendAttachments).length > 0) {
