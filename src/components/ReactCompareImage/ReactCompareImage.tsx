...
const ReactCompareImage: React.FC<Props> = ({
  ...
  const handleSliding = useCallback(
    (event) => {
      ...
      // If there's a callback function, invoke it every time the slider changes
  ...
  return (
    <>
      {skeleton && !allImagesLoaded && (
        <Box sx={{...styles.container}}>{skeleton}</Box>
      )}
      ...
      >
        ...
        <Box sx={styles.slider}>
          <Box sx={styles.line} />
          {handle ? (
            <Box sx={styles.handleCustom}>{handle}</Box>
          ) : (
            <Box sx={styles.handleDefault}>
        ...
        {/* labels */}
        {leftImageLabel && (
          <Box sx={styles.leftLabelContainer}>
            <Box sx={styles.leftLabel}>{leftImageLabel}</Box>
          </Box>
        )}
        {rightImageLabel && (
          <Box sx={styles.rightLabelContainer}>
            <Box sx={styles.rightLabel}>{rightImageLabel}</Box>
          </Box>
        )}
      </Box>
    </>
